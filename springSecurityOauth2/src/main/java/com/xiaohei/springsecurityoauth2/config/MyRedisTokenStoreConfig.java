...
@Component
public class MyRedisTokenStoreConfig implements TokenStore {
...
//    private RedisTokenStoreSerializationStrategy serializationStrategy = new JdkSerializationStrategy();
    ...
    public MyRedisTokenStoreConfig(RedisConnectionFactory connectionFactory) {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        //新加的key
        byte[] newAuthKey = serializeKey(AUTH + getApprovalKey(authentication) + ":" + token.getValue());
        RedisConnection conn = getConnection();
        try {
            conn.openPipeline();
            ...
            conn.stringCommands().set(newAuthKey, serializedAccessToken);
            ...
            if (token.getExpiration() != null) {
                ...
                //设置新key的过期时间
                conn.expire(newAuthKey, seconds);
            }
